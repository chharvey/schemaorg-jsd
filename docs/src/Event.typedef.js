/**
 * @summary An event happening at a certain time and location, such as a concert, lecture, or festival.
 * @see http://schema.org/Event
 * @typedef {Thing} Event
 * @property {string=} endDate The end date and time of the item (in ISO 8601 date format).
 * @property {(string|Place|PostalAddress)=} location The location of for example where the event is happening, an organization is located, or where an action takes place.
 * @property {string=} startDate The start date and time of the item (in ISO 8601 date format).
 */
